...
function ScrollGlobe({ sections, globeConfig = defaultGlobeConfig, className }: ScrollGlobeProps) {
    ...
    const [currentPos, setCurrentPos] = useState<any>(null); // State for current position
    ...
    // Initial position
    useEffect(() => {
        setCurrentPos(calculatedPositions[0]);
    }, [calculatedPositions]);

    // Scroll tracking container-based
    const updateScrollPosition = useCallback(() => {
        ...
        // Container-relative section detection
        ...
        sectionRefs.current.forEach((ref, index) => {
            if (ref) {
                ...
                // Calculate center relative to viewport implies relative to container if container is in viewport
        ...
        setCurrentPos(calculatedPositions[newActiveSection]);
    ...
    return (
        <div
            className={cn(
                "relative w-full h-full overflow-hidden bg-background text-foreground group/container",
                className
            )}
        >
            {/* 1. Global Background/Globe - Fixed relative to container */}
            <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden">
                <div
                    className="absolute transition-all duration-[1400ms] ease-[cubic-bezier(0.23,1,0.32,1)]"
                    style={{
                        left: currentPos ? `${currentPos.left}%` : '50%',
                        top: currentPos ? `${currentPos.top}%` : '50%',
                        transform: `translate(-50%, -50%) scale(${currentPos ? currentPos.scale : 1})`,
            ...
            {/* 2. UI Elements */}
            <div className="absolute top-0 left-0 w-full h-0.5 bg-gradient-to-r from-border/20 via-border/40 to-border/20 z-50">
                <div
            ...
            <div className="hidden sm:flex absolute right-2 sm:right-4 lg:right-8 top-1/2 -translate-y-1/2 z-40 pointer-events-none">
                <div className="space-y-3 sm:space-y-4 lg:space-y-6 pointer-events-auto">
            ...
            <div
                ...
                className="relative w-full h-full overflow-y-auto z-10 no-scrollbar scroll-smooth"
            >
                {sections.map((section, index) => (
                    <section
                        key={section.id}
                        ...
                        className={cn(
                            "relative min-h-screen flex flex-col justify-center px-4 sm:px-6 md:px-8 lg:px-12 py-12 sm:py-16 lg:py-20 pointer-events-none",
                    ...
                    >
                        <div className={cn(
                            "w-full max-w-sm sm:max-w-lg md:max-w-2xl lg:max-w-4xl xl:max-w-5xl will-change-transform transition-all duration-700 pointer-events-auto",
