...
const ImagesScrollingAnimation = ({ scrollContainerRef }: { scrollContainerRef?: React.RefObject<HTMLDivElement> }) => {
    const container = useRef<HTMLDivElement>(null)
    const { scrollYProgress } = useScroll({
        target: container,
        container: scrollContainerRef, // Bind to modal if provided
        offset: ["start start", "end end"],
    })

    return (
        // Removed root prop to prevent hijacking window scroll. 
        // If inside a modal, native scroll is usually sufficient or Lenis needs to track the modal div.
